...
| 4.6 | ~~Portabilidade do tenant~~ **feito** | `GET /api/v1/projects/:projectId/export`. Tabelas descobertas do BANCO (`sqlite_master` + `PRAGMA`), não de lista — export incompleto é pior que nenhum, porque parece completo, e há teste que cria tabela em tempo de execução para provar que ela entra. Credenciais (`api_keys`, `auditor_tokens`) ficam de fora por decisão explícita. `sha256` sempre (integridade) e assinatura **Ed25519** quando a chave existir (origem) — assimétrica, e não HMAC, porque com chave simétrica quem verifica também forja, e o recipiente do export é o cliente, às vezes o sucessor dele. A pública é servida em `GET /api/v1/public/export-public-key` e versionada em `docs/export-public-key.json`; a privada nunca sai do Worker. **Configurada em produção.** Isolamento pelo `projectAccessMiddleware`, com teste de vizinho. Ver `docs/portabilidade.md` |
