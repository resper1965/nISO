                                        ...
                                        <div class="phase-details ${isPhExpanded ? 'expanded' : ''}" id="p-details-${ph.phase_number}">
                                            ...
                                            ${(window.PHASE_PLAYBOOKS && window.PHASE_PLAYBOOKS[ph.phase_number]) ? `
                                                <div class="playbook-card" style="margin-bottom:1.25rem; padding:12px; background:rgba(0,173,232,0.04); border-left:3px solid var(--accent); border-radius:10px; font-size:0.75rem">
                                                    <div style="font-weight:500; color:var(--accent); margin-bottom:4px; font-family:'Montserrat',sans-serif; text-transform:uppercase; letter-spacing:0.5px; font-size:0.7rem">Diretriz do Consultor: ${escapeHTML(window.PHASE_PLAYBOOKS[ph.phase_number].obj)}</div>
    ...
    window.renderJourneyDiagnosticPanel = function(jIdx, projectId) {
        ...
        if (jIdx === 0) {
            ...
            if (answers.sponsor !== 'sim') {
                suggestionsList += `<li><strong>Sem sponsor executivo ativo:</strong> Risco crítico para a certificação (Requisito Cl 5.1). Recomendamos formalizar a nomeação do comitê com ata assinada.</li>`;
            }
            if (answers.budget === 'nao') {
                suggestionsList += `<li><strong>Sem orçamento de segurança:</strong> Risco ao cronograma de contratação da certificadora. Recomenda-se reservar verba de auditoria externa preventiva.</li>`;
            }
            ...
            if (answers.encryption !== 'sim') {
                suggestionsList += `<li><strong>Criptografia em repouso pendente:</strong> Essencial para proteger dados contra quebra física (A.8.24). Habilitar criptografia transparente no banco de dados.</li>`;
            }
            if (answers.backup === 'nunca') {
                suggestionsList += `<li><strong>Sem testes de restauração:</strong> O auditor irá solicitar registros dos testes dos últimos 12 meses. Configure teste de simulação mensal (A.8.13).</li>`;
            }
            if (answers.mfa !== 'todos') {
                suggestionsList += `<li><strong>MFA parcial ou inativo:</strong> Vulnerabilidade de controle de acesso (A.5.15). Ativar MFA obrigatório para toda equipe e e-mail corporativo.</li>`;
            }
        }
        else if (jIdx === 3) {
            ...
            if (answers.dsr === 'nao') {
                suggestionsList += `<li><strong>Sem canal para titulares:</strong> OBRIGATÓRIO implementar canal de requisição DSR e registrar tempos de resposta no relatório de privacidade (Cl 7.3.1).</li>`;
            }
        }
        else if (jIdx === 4) {
            ...
            if (answers.audit !== 'sim') {
                suggestionsList += `<li><strong>Auditoria Interna pendente:</strong> Requisito obrigatório para o Estágio 1 da certificação. Agende a auditoria antes da auditoria externa (Cl 9.2).</li>`;
            }
            if (answers.review === 'nao') {
                suggestionsList += `<li><strong>Ata de Revisão pela Direção pendente:</strong> Documento chave exigido pelo auditor no Estágio 2. Agende reunião e registre a ata (Cl 9.3).</li>`;
            }
        }
        else if (jIdx === 5) {
            ...
            if (answers.capa === 'nao') {
                suggestionsList += `<li><strong>Sem processo CAPA estruturado:</strong> Essencial para comprovar melhoria contínua (Cl 10.1). Registre qualquer desvio na aba Ações Corretivas.</li>`;
            }
        }
        ...
        return `
            <div style="margin: 0 1rem 1rem 1rem; padding: 12px; background: rgba(0, 173, 232, 0.04); border: 1px dashed rgba(0, 173, 232, 0.2); border-radius: 8px;">
                <div style="font-size: 0.7rem; text-transform: uppercase; color: var(--accent); font-weight: 700; margin-bottom: 6px; text-align: left; display: flex; justify-content: space-between;">
                    <span>Diagnóstico de Conformidade (nISO Consultor)</span>
